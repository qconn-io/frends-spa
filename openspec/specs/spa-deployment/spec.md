...
Slug-scoped deployment of a validated single-file SPA bundle: the local `npm run deploy`
CLI builds and uploads the bundle to the protected `Deploy SPA Bundle` Process targeting a
required slug, and the Process validates the slug and bundle, writes a versioned file into
the slug's subdirectory, and flips that slug's pointer file last as the atomic commit.
