...
Compiles to a self-contained `dist/index.html` (all JS/CSS inlined) served from a public Frends HTTP Trigger endpoint. The deploy and serve triggers use their root-level route templates (`/spa-deploy`, `/ui/{slug}`); Frends does not prepend `/api` to them. App data calls can still use root-relative `/api` paths by convention. Built with Vue 3, TypeScript, and Vite.
...
3. **Serve**: The `Serve SPA` process resolves the slug from `GET /ui/{slug}`, reads that slug's pointer, and returns the active HTML bundle with `ETag`/`304` caching.
...
- **Permissions** to import Processes, install Task packages, set Environment Variables, and deploy HTTP triggers.
...
| `FRENDS_DEPLOY_URL` | Deploy | Deploy endpoint (e.g., `https://<agent>/spa-deploy`; no `/api` prefix). |
| `FRENDS_DEPLOY_KEY` | Deploy | Deploy credential sent as `x-api-key` by `scripts/deploy.mjs`. **Keep secret.** |
